...
> (4 Jul, `\bIndividual \bquota reached`, limit≠exit, `remainingFraction` absent)**. **Tak ada lagi ADR Proposed.**
...
dua verifikasi terakhir tertutup dgn limit ASLI: **(CC, 4 Jul pagi)** pesan `You've \bhit your session limit` + limit≠exit;
**(agy, 4 Jul)** pesan TUI `\bIndividual \bquota reached` + limit≠exit + sinyal exhaustion LS (`remainingFraction` absent) +
...
ASLI `⚠ \bIndividual \bquota reached … Resets in <Xm Ys>.` + Error ID; **agy limit≠exit** (tetap hidup di prompt);
...
**terverifikasi ASLI = TETAP HIDUP di prompt** (limit≠exit, seperti CC) setelah pesan `\bIndividual \bquota reached` →
...
`\bIndividual \bquota reached` (limit-watcher, G-19, terbukti fire) → LIMIT_HIT baru → jadwalkan ulang `probe`/resume di `reset_at`
...
| Tanggal | Perubahan |
|---|---|
| 2026-07-17 (sesi Windows baru, I-35 residual + I-36 — **realisasi, bukan ADR**) | **Dua realisasi minor (bukan ADR; tak ubah arsitektur/keputusan struktural apa pun).** **(1) I-35 residual — guard status cabang `probe`:** sebelum fix, job `probe` fire tanpa menanyakan `session.status` → job stale (sesi sudah `RUNNING`, mis. resume manual/race) bisa meng-inject sesi yang tak pernah dikonfirmasi limit (keluarga F-1). Guard `status!=='LIMIT_HIT'`→no-op teraudit menutupnya. **Bukan** implementasi "probe verifikasi eksplisit" (poin 2 entri I-35 sebelumnya) — itu tetap butuh keputusan desain terpisah (overload semantik `probe` vs job `kind:'verify'` baru). **(2) I-36 — gate higiene literal kanonik:** 61 baris/12 file diperbaiki (escape `\b` untuk prosa; by-index `CC_LIMIT_PATTERNS[N]` untuk data-record `evidence:"..."` supaya tak salah-representasi nilai tercatat) + gate permanen `test/no-canonical-limit-literals.test.ts` (pakai `matchLimit`/`matchAgyLimit` produksi, bukan salinan regex). Dua pengecualian struktural bermarker `gate:allow-canonical-literal`: regex-array-literal `AGY_LIMIT_PATTERNS` (source niscaya memuat teks targetnya sendiri) + `notifier.ts` title (string user-facing yang sengaja meniru bahasa kanonik; risikonya sudah ditutup di lapis deteksi I-35, bukan disembunyikan dari user). **G-46** mencatat 3 jebakan mekanis nyaris-lolos saat membangun gate ini sendiri (JS `'\b'`=backspace bukan literal 2-karakter; escape ke baris definisi regex mengubah semantik produksi; satu-evidence-per-baris melewatkan baris ber-evidence-ganda). Keduanya reversible, keduanya diverifikasi negative-control konkret. Dampak docs: ISSUES (I-35 sub-item guard→✅, I-36→✅), GOTCHAS (**G-46**), CLAUDE.md/README (475→570 test), CONTEXT. |
| 2026-07-17 (sesi Windows, I-35 — **korroborasi deteksi**) | **Keputusan MINOR reversible (bukan ADR baru; owner Ziffan): sinyal limit dari OUTPUT untuk sesi CC WAJIB dikorroborasi snapshot usage — ambang `0.85`, kesegaran `5 menit`, hook `StopFailure` BYPASS.** Dipicu insiden live (3 LIMIT_HIT palsu dalam satu sesi, **G-45**; metrik FP `PROJECT.md` §1 "<1 per 100 sesi" meleset **orde besaran**). Owner memilih **suppress di titik DETEKSI** (bukan di actuation) + ambang 0.85 — duduk di tengah jurang antara FP terukur (0.55) dan limit asli terukur (≥0.94, T-6). **Kenapa BUKAN ADR:** ini menyempitkan *kapan* fallback output ADR-001 dipercaya, **tidak** mengubah sumber deteksi maupun hierarkinya (hook tetap PRIMER, output tetap fallback) — sekelas **I-31** (grace-window OUTPUT-CC 5s) yang juga issue-level. Konstanta = tuning reversibel, bukan struktur. **Owner boleh menaikkannya jadi ADR bila menilai ini menggeser garis ADR-001** — flagged, bukan diputuskan diam-diam. **Firewall ADR-013 MENGUAT** (lebih sedikit aksi diturunkan dari isi output). Scope: **CC-only + OUTPUT-only**; agy tak disentuh (nol hook + G-35 snapshot LS stale in-sesi → korroborasi tak andal). **Paruh yang DISETUJUI TAPI BELUM DIBANGUN:** probe verifikasi eksplisit — framing awal Opus ("menunggangi mesin async yang ada") **KELIRU**: cabang `probe` tak pernah menanyakan apakah sesi limit → akan `enqueue resume`+inject, persis bahaya yang dicegah; butuh guard status (= perbaikan korektness berdiri sendiri, keluarga F-1). Jaring FN sementara = repaint self-healing (ada test) **atas asumsi belum terbukti** bahwa CC me-repaint saat limit-asli-dan-diam (bukti G-37 dari skenario pasca-inject) → **residual terbuka di I-35**. Dampak docs: DECISIONS (baris ini), ISSUES (**I-35** + **I-36** + konvensi wajib), GOTCHAS (**G-45**), CLAUDE.md/README (474 test), CONTEXT. |
| 2026-07-17 (sesi Windows, M5.5 — **penundaan atas bukti**) | **Keputusan MINOR reversible (bukan ADR baru; owner Ziffan): Windows = `acca daemon` MANUAL dulu — realisasi ADR-021 DITUNDA, keputusannya TIDAK dibalik.** Slice M5.5 dihentikan **sebelum satu baris kode ditulis** setelah probe empiris (service uji LocalSystem, read-only, di-uninstall bersih) membuktikan **Windows Service ≠ sesi user**: `whoami`=`nt authority\system`, `homedir`=`…\systemprofile`, **`sameDbAsUser:false`**, `resolvedDbExists:false` (daemon akan migrasi + **buat DB kosong baru**), **`credentials.exists:false`** → `claude`/`agy` ter-spawn tak terautentikasi. **Premis ADR-021 bentrok ADR-005** ("mewarisi sesi login mesin") — tabrakan yang tak terlihat saat spec ditulis karena ADR-021 tak memeriksa **identitas akun** service. **Kelas kegagalan terburuk:** senyap + **lolos AC-M5-2 yang ada** (`sc query` RUNNING sementara produk mati; `acca status` tampak normal karena CLI baca DB langsung, G-42) → **AC-M5-2 diperkuat** (wajib: daemon baca `acca.db` SAMA + CLI ter-spawn TERAUTENTIKASI). **Ditolak:** LocalSystem+`<env>` pin (atas dasar **keamanan**, bukan teknis — spawn agent CLI arbitrer sbg SYSTEM = eskalasi privilege sbg fitur; + risiko DPAPI user-scope) dan `<serviceaccount>` WinSW (menuntut **password plaintext di XML** per `sample-allOptions.xml` resmi). **Jalur as-user = 4 ketidakpastian belum teruji** (password owner; `SeServiceLogonRight` via `secedit` = mutasi kebijakan keamanan; MS: profil user **tak** otomatis di-load untuk service; **session-0** PTY/ConPTY + auth `claude` belum pernah dibuktikan) → **I-33**. **Linux TAK kena** (`systemd --user`+linger jalan sebagai user; asimetri fundamental — Windows tak punya padanan) → **M5.4 = jalur always-on sejati**. ADR-021 **dianotasi** (isi immutable tak diedit) + MILESTONES M5.5 ditandai DITUNDA + syarat buka lagi. Pin WinSW ADR-025 **tetap sah** untuk dipakai saat dibuka lagi. Reversible; tak ada ADR di-supersede. Dampak docs: DECISIONS (anotasi ADR-021 + baris ini), ISSUES (**I-33** P1 + **I-34** kelas gate artefak), GOTCHAS (**G-43**, **G-44**), MILESTONES M5.5, README, CLAUDE.md, CONTEXT. |
...
| 2026-07-04 | **ADR-001 di-ACCEPT (locked)** — verifikasi terakhir tertutup dgn limit ASLI kedua CLI: CC (`You've \bhit your session limit`, 4 Jul pagi) + **agy** (kuota 5-jam dihabiskan terkontrol → pesan TUI `⚠ \bIndividual \bquota reached … Resets in <Xm Ys>` + Error ID; **agy limit≠exit** tetap hidup; sinyal exhaustion LS `remainingFraction` **absent**; limit agy **soft** bila credit aktif = fallthrough senyap; print-mode kosong saat limit). **Tak ada lagi ADR Proposed — set ADR lengkap-terkunci.** **ADR-014 dianotasi** (keputusan tak berubah): jalur inject-continue agy **tak lagi provisional** (agy hidup di prompt saat limit). Header status + ADR-001 status + progres diperbarui. Dampak docs: GOTCHAS G-16..G-19, RESEARCH §2b/§4b, MILESTONES M3d.4/M3d.8, CONTEXT. Sumber: scratchpad FINDINGS F4-F12. |
