...
- **Terakhir diupdate:** 2026-07-17 (sesi **Ubuntu**, dgn user — `/session-start` → **M5.4 systemd DITUTUP: SANDBOX + LIVE parsial**) — **585 test hijau** (+15; 2 skip POSIX). Sesi ini di mesin yang benar (Linux) untuk slice yang owner sengaja tunda ke Ubuntu.
  ...
  **(2) Sub-task 2 — LIVE di mesin ini (parsial):** install→`active (running)`; **auto-restart AC-M5-1** SIGKILL→restart PID baru ~5s (`NRestarts=1`, <30s) — **catatan G-47:** `Restart=on-failure` TAK restart pada SIGTERM/exit-bersih, uji WAJIB SIGKILL; **same-DB (kontras I-33)** `acca status`→"daemon HIDUP" = CLI & service systemd berbagi `acca.db` sama (di Linux user-sama → nol split-brain LocalSystem); `Linger=yes`. **Residual (butuh tangan owner):** logout→login survival + **reboot** auto-start + AC-M5-3 (job LIMIT_HIT pending fire pasca-boot). Daemon **dibiarkan terpasang** (keputusan owner) — build ter-proteksi I-35.
