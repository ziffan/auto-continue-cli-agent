...
│   ├── Detector             — kenali LIMIT_HIT dari exit code | pola output | transcript
│   │                          (Claude Code: JSONL; Antigravity: .pb protobuf → andalkan exit/PTY)
...
│   │                          usage; Antigravity: probe kuota) — per adapter
...
| Resume (Claude Code / Antigravity) | `claude --resume <id>` / `agy --conversation <id>` | Terverifikasi v2.1.198 / v1.0.15 |
...
- `sessions` — id, tool, session_id, cwd, pid, status (`RUNNING|LIMIT_HIT|WAITING|RESUMED|EXITED|BLOCKED|FAILED`),
  detected_at, reset_at, reset_source (`exact|heuristic|backoff`), created_at, updated_at.
