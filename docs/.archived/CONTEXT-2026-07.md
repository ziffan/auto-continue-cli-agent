# CONTEXT — arsip riwayat sesi (M1 s.d. 2026-07-18)

> Dipindah dari `docs/CONTEXT.md` saat pelangsingan 18 Jul (nol hard-delete). Greppable, di luar jalur session-start.
...
- **Terakhir diupdate:** 2026-07-18 (sesi **Windows, dgn owner** — `/session-start` → **I-35 DITUTUP PENUH**: probe verifikasi eksplisit `kind:'verify'`) — **626 test hijau** (+5 dari 621; 2 skip POSIX). Owner tunda M-remote → garap I-35 sisa (P1, jalur firewall paling sensitif, owner hadir). Baseline Step 0 sinkron origin 0/0, tree bersih.
  **Keputusan desain (di-lock owner sesi ini):** residual I-35 = job **`kind:'verify'` baru** (BUKAN overload `probe` — cabang `probe` sudah di-guard `status==='LIMIT_HIT'`, kebalikan verify → overload mekanis buntu). Semantik owner: **verify→habis→latch SAJA** (mesin reset/probe normal ambil alih, tak langsung resume) + **delay 2,5 mnt** (>lag probe ~2mnt/T-6). Bukan ADR (mekanisme di dalam ADR-013/019, reversible, sekelas I-31); spec di-lock via 2 pertanyaan + review inline.
  **(1) Implementasi (Opus inline, Tier-1 state-machine+firewall):** `types.ts` JobKind+`verify`; **migrasi 0003** widen `CHECK(kind)` via table-rebuild (upgrade v2→v3 **diuji konkret**: baris lama utuh, verify diterima, bogus ditolak, FK tegak); `process-wrapper.ts` `onUsageContradiction` enqueue verify @+150s + `verify_scheduled` + `notifyDaemonRearm`; `scheduled-jobs.ts` **`hasPendingKind`**; `supervisor.ts` **cabang dispatch `verify`** (guard tool CC-only + guard status RUNNING-alive → probe → `verify_fp_confirmed`/`verify_latched_real_limit`/`verify_unreadable`). **Firewall ADR-013 MENGUAT** (keputusan latch murni dari probe usage, bukan isi output).
  **(2) Blind-spot penulis=reviewer di-flag & ditutup:** `onUsageContradiction` menyala **per baris** → prosa multi-literal (Read `patterns.ts`/docs, skenario inti I-35/I-36) memicu **N job verify** → **dedup `hasPendingKind`** (satu verify/episode; latch men-dedup sisi habis). **G-55** (enqueue-per-line storm) + **G-56** (`git checkout` tracked-file over-revert — kerja `process-wrapper.ts` sempat hilang saat NC, **di-re-apply penuh** + verifikasi marker).
  **Verifikasi (Opus sendiri):** `npm run check` typecheck+lint+**626 test** (2 skip POSIX). **+5 test** (4 dispatch + 1 integrasi PTY-nyata wiring+dedup). **3 NC terbukti KONKRET:** (a) enqueue dimatikan → wiring merah; (b) paksa `hasAvailable=true` → **hanya** test latch merah (3 lain hijau = isolasi); (c) bypass dedup → 2 verify job. Tier-1 self-review PASS.
  **Batas LIVE jujur:** verify-under-real-limit end-to-end = kelas I-15 (butuh limit CC asli + user) — teruji di sini via probe ter-stub + wiring PTY-nyata; latch-vs-FP di limit NYATA menunggu episode limit (dogfood, sama batas I-31/I-15).
  **Docs:** ISSUES (**I-35 DITUTUP PENUH** — sub-item probe-verifikasi ✅), GOTCHAS (**G-55/G-56** + Change Log), CHANGELOG (verify di entri I-35), CLAUDE.md/README (621→626), CONTEXT (ini). **Commit + push:** lihat git log.
  **Next konkret:** (1) **M-remote tier A** (Notifier→Telegram — milestone berikutnya per roadmap; butuh bot-token infra-secret + gate dependency grammy 1.44.0 + egress live `api.telegram.org` → owner hadir). (2) Opportunistik non-gate: verify-under-real-limit LIVE saat limit CC berikutnya. (3) sisa P3 (C-5, F-4/5/6) low-value ADR-013-sensitive.

- **Terakhir diupdate:** 2026-07-18 (sesi **Windows, otonom** — owner tinggalkan terminal → **backlog I-32 + I-8/I-17 DITUTUP**) — **621 test hijau** (+6 dari 615; 2 skip POSIX). Owner tunda **M-remote** (fungsi remote Claude Code sudah cukup) → garap backlog. Baseline Step 0 sinkron origin 0/0, tree bersih.
  **(1) I-32 (P2) DITUTUP — engine backup → SQLite online backup API.** `backupDatabase` diubah sinkron (`wal_checkpoint(TRUNCATE)`+`copyFileSync` file-utama, rawan salinan half-written saat daemon nulis konkuren) → **async `db.backup(dest)`** (better-sqlite3 12.11.1) — page-by-page dengan lock benar, **concurrency-safe by construction** → race copy-vs-checkpoint (risiko b) lenyap. Koneksi sumber tetap terbuka saat transfer, ditutup `finally`; `integrity_check` fail-safe dipertahankan; `pruneSnapshots` tak berubah. Caller: `scripts/backup.js` (top-level await ESM) + `test/backup.test.ts` (semua `await`, error-path `rejects.toThrow`). **+1 test (T5** concurrency). **Jujur (G-53):** race korupsi lama = nondeterministik → **T5 = scenario/kapabilitas, BUKAN negative-control keras** (uji empiris: pendekatan lama pun lolos T5 di writer-idle krn checkpoint sempat flush). Fix tetap sah (hapus race by construction).
  **(2) I-8/I-17 (P2) DITUTUP — proximity: wiring sudah ada (docs drift) + gap spam ditutup.** Temuan: engine `usage-monitor.ts` + wiring `supervisor.ts` (`startUsageMonitor`) + `daemon.ts` (`startUsageMonitor: true`, interval 120s) + 2 test **SUDAH ter-commit** sebelum sesi ini; ISSUES I-8 belum ditandai → dikoreksi. **Gap nyata (G-54):** `proximityNotifications` stateless di-deliver **tiap tick** (~2 mnt) selama sesi di atas ambang → spam (~30 notif/jam @95%). Fix: **`createProximityGate`** (rising-edge dedup, state per `(tool,kind)`, clear per-tool scoped) — notif hanya saat window BARU melewati ambang. Threshold logic tetap satu tempat (`proximityCandidates`). **+5 test** (4 gate + 1 monitor multi-tick). **Negative control terbukti** (bypass gate → deliver 3×).
  **Verifikasi (Opus sendiri):** `npm run check` typecheck+lint+**621 test** (2 skip POSIX). Tier-1 self-review PASS kedua slice (backup data-integrity path + proximity firewall G-9; keduanya nol egress baru, reversible). Negative control dijalankan konkret (I-8 terbukti; I-32 dinyatakan jujur BUKAN hard-NC).
  **Docs:** ISSUES (I-32 ✅ + I-8 ✅ ditutup, konteks asli ke `<details>`), GOTCHAS (**G-53** online-backup + **G-54** stateless-engine-in-periodic-loop + Change Log), CHANGELOG (Added proximity + Changed backup engine), CLAUDE.md/CONTEXT (615→621). **Commit + push:** lihat git log.
  **Next konkret:** (1) **I-35 sisa** (P1, terbuka) — owner condong **job `kind:'verify'`** (bukan overload `probe`); catatan owner: acca masih uji-coba, DB boleh reset → tak wajib migrasi hati-hati (kolom `kind` nambah nilai, baris lama tetap valid saat schema di-rebuild). Butuh owner hadir (jalur firewall paling sensitif — insiden live 17 Jul). (2) **M-remote tier A** saat owner siap (butuh bot-token + gate grammy + egress live). (3) sisa P3 (C-5, F-4/5/6) low-value ADR-013-sensitive.

- **Terakhir diupdate:** 2026-07-18 (sesi **Windows asli**, dgn owner — `/session-start` → **M5.5 DITUTUP PENUH → M5 TUTUP PENUH**) — **615 test hijau** (+~28 dr baseline Linux 585; +18 gate task-scheduler +2 ps1-encoding; drift lintas-OS). claude polos, acca mati (owner konfirmasi → aman baca docs).
  **Baseline:** Step 0 sinkron origin (lokal ahead 2 commit ADR-026, bersih). **KOREKSI merah pra-eksisting:** `npm run check` menyingkap `docs/DECISIONS.md:33` memuat literal CC kanonik **tak ter-escape** (dari commit reflow `b925455` "perbaikan spasi" yang meng-un-escape baris, lalu 2 commit ADR-026 tak re-run check) → **HEAD merah di `main` sebelum kerja saya**; diperbaiki (escape `\b`, konvensi I-36). Bukan kerjaan saya, tapi disurface + ditutup.
  **(1) Sub-task 1 [SANDBOX] — gate DULU (I-34), lalu render:** `test/task-scheduler-xml.test.ts` (18 test) DIDESAIN sebelum template — well-formed + **`--`-in-comment** + ASCII + LogonTrigger/Principal(LeastPrivilege)/Hidden/PT0S/battery-safe/IgnoreNew/**watchdog Repetition**/RestartOnFailure + render nol-remnant + substitusi-coverage `.ps1`. Render: `deploy/windows/acca-daemon.task.xml` (placeholder `{{TOKEN}}` bukan `<TOKEN>` — hindari tabrakan tag XML) + `scripts/install-windows.ps1`/`uninstall-windows.ps1` (register via `Register-ScheduledTask -Xml`, ASCII G-44, idempoten). **3 negative control terbukti** (eskalasi 1→2→4→5).
  **(2) DUA temuan LIVE mengubah artefak (I-34 membayar 2×):** **(a) G-50** — gate `xmlTagBalance` buatan-sendiri LOLOS-kan XML, tapi `System.Xml`/`Register-ScheduledTask` (parser sungguhan, dijalankan di Win asli) **MENOLAK** komentar ber-`--` (`systemd --user`) → task **gagal register** → gate diperkuat cek `--`-in-comment + XML diperbaiki. **(b) G-49** — `RestartOnFailure` (Interval PT1M Count 3) LIVE **TAK me-restart** daemon di-`taskkill /F` (nol restart 100s; `LastTaskResult=1`, task→Ready) → redesign ke **watchdog repetisi** (`LogonTrigger`+`Repetition PT1M`+`IgnoreNew`) → kill→restart **~61s** terverifikasi (temp time-trigger task, tak perlu logout berulang). Floor PT1M → **AC-M5-2 restart target diamandemen <30s→<90s** (keputusan owner; konsisten ADR-026 always-on-terdegradasi laptop). **G-51** (git checkout tak revert untracked).
  **(3) ✅ LIVE AC-M5-2 (mesin asli, user `lab2026zf\ziffa`) — hampir penuh:** task register per-user nol-admin (LeastPrivilege/InteractiveToken/Hidden/PT0S/battery-safe) + daemon owner=**ziffa** (bukan SYSTEM, **I-33-proof by construction+empiris**) + **same-`acca.db`** (`acca status`→HIDUP di PID hasil-spawn task, kontras I-33 DB-split) + `.credentials.json` terlihat + **@logon trigger FIRE terkonfirmasi** (owner logout→login → daemon start otomatis, `LastRunTime`=waktu login, owner=ziffa) + **watchdog restart ~65s** + **nol jendela konsol**.
  **(4) TEMUAN LOGON + FIX (G-52) — `Hidden=true` TAK cukup:** logout/login owner menyingkap **jendela `PseudoConsoleWindow` node TERLIHAT** walau `Hidden=true` (mata owner; `Hidden` cuma sembunyikan task dari UI, bukan jendela proses). Klaim commit `8c393f3` "nol-jendela (MainWindowHandle=0)" **SALAH** — detektor naif lolos-palsu (jendela dimiliki ConPTY, bukan MainWindow). **Fix:** node dijalankan via **`conhost.exe --headless`** (nol jendela; conhost=induk → task "running" → `IgnoreNew` tetap sah). Detektor benar = `EnumWindows`+class `PseudoConsoleWindow`. **Terverifikasi:** parent=conhost, `EnumWindows`→NONE, watchdog-restart ~65s→NONE, `acca status`→HIDUP. **Reproduksi murah:** `Start-ScheduledTask` on-demand **mereproduksi jendela logon** → iterasi fix tanpa logout berulang. VBScript+wscript ditolak (deprecated).
  **(5) ✅ AC-M5-2 DITUTUP PENUH (logout/login final owner, 18 Jul):** daemon auto-start pid 420 owner=ziffa **parent=conhost**, **nol tampilan terminal** (mata owner "tidak ada tampilan terminal" + `EnumWindows`→NONE), `acca status`→HIDUP, `LastRunTime`=waktu login, watchdog armed PT1M. **→ M5.5 ✅ PENUH → M5 ✅ TUTUP PENUH (Linux + Windows).** Catatan (b) spawn-terautentikasi: KAPABILITAS terbukti (daemon=user → creds terlihat, basis sama Linux); resume-nyata end-to-end = kelas I-15 (butuh limit asli). Task terpasang (Action=conhost, armed @logon).
  **Verifikasi (Opus sendiri):** `npm run check` typecheck+lint+**615 test**. Tier-1 self-review PASS (privilege-boundary least-priv; spawn-path). **Docs:** MILESTONES (AC-M5-2 ✅ + M5.5 ✅ PENUH + M5 header TUTUP PENUH), ISSUES (**I-34 DITUTUP**), GOTCHAS (**G-49/50/51/52** + Change Log), DECISIONS (fix L33), CONTEXT (ini), CLAUDE.md/README (585→615). **3 commit:** `8c393f3` (SANDBOX+LIVE-parsial) + `6202455` (conhost no-flash) + closure (ini).
  **Next konkret:** **M5 SELESAI** → (1) **M-remote tier A** (Notifier→Telegram — milestone berikutnya per roadmap; butuh bot-token infra-secret + gate dependency grammy + egress live `api.telegram.org` → owner hadir); (2) **I-35 sisa** (probe verifikasi eksplisit, P1, keputusan desain owner: overload `probe` vs job `verify`); (3) I-32 online-backup API saat wiring LIVE.

- **Terakhir diupdate:** 2026-07-17 (sesi **Cowork/Windows**, dgn owner — riset + keputusan arsitektur, **docs-only**) — **ADR-026 di-LOCK: I-33 di-unblock via ganti jalur deployment Windows. Nol kode; 585 test tak berubah.**
  **Pemicu:** owner minta pandangan untuk menuntaskan **M5.5 yang ditunda (I-33)** + riset apakah `acca daemon` bisa pakai pendekatan **autostart-app seperti CodexBar Windows** (`nesszer/Win-CodexBar`, Tauri, autostart `HKCU\…\Run`, jalan sebagai user — bukan service).
  **Temuan inti (verdict):** **bisa, dan itu jalur yang benar** — bukan workaround. **I-33 seluruhnya = mismatch identitas akun** (Windows Service default LocalSystem → `acca.db` beda + creds putus → mati senyap). Mekanisme apa pun yang jalan **sebagai user login di sesi interaktif** menghilangkannya **by construction**: `$HOME`/`acca.db`/`.credentials.json` benar + **session-0 PTY tak relevan** (daemon di sesi interaktif user, spawn `claude` persis seperti `acca daemon` manual hari ini). Ini **analog Windows dari `systemd --user`** (M5.4 LIVE) tanpa linger. **Koreksi ADR-021:** penolakan "Task Scheduler" di ADR-021 spesifik trigger **ONSTART (at-boot)**; trigger **AT-LOGON per-user** = kategori berbeda yang tak pernah dievaluasi; klaim "Task Scheduler tanpa auto-restart" **tak akurat** (Task Scheduler punya restart-on-failure eksplisit — yang tanpa restart = Startup-folder/`Run`-key).
  **✅ ADR-026 ditulis via skill `adr` + di-LOCK (owner Ziffan):** deployment Windows MVP = **autostart per-user (Task Scheduler trigger "At log on", run-hidden, restart-on-failure)**, registrasi via template XML + skrip `.ps1` (nol dep npm). **Men-supersede sebagian** klausa Decision Windows ADR-021 (target MVP laptop) + fallback "`acca daemon` manual" (I-33 opsi c). Windows Service **tak dihapus** → di-demote ke opsi host non-laptop selalu-login (tetap ter-blok I-33 utk kasus itu); **ADR-025 pin WinSW = dorman tapi sah**. **Konsekuensi negatif jujur & diterima:** TIDAK truly always-on (tak lintas-logout, tak at-boot-pra-login) — sudah direlakan owner utk profil laptop (ADR-007); **truly always-on tetap = Linux M5.4**. **Ditolak:** Task Scheduler "run whether logged on or not" (butuh password/S4U → reintroduce I-33) + Startup-folder/`Run`-key polos (nol restart + flash konsol; analogi CodexBar **lemah sbg bukti keandalan** — ia monitor read-only nol-requirement-unattended, bar-nya jauh di bawah acca yang spawn agent + inject PTY).
  **Docs diubah (semua via `Edit`, disk-direct — nol git dari sandbox):** DECISIONS (ADR-026 baru + anotasi status ADR-021 [immutable, isi tak diedit] + header status + Change Log), ISSUES (**I-33 → RESOLVED-BY-PATH-CHANGE**, blocker MVP ditutup; residual jalur-Service di-defer P3), MILESTONES (**AC-M5-2 diperkuat**: bukti wajib `acca.db` SAMA + `claude` ter-spawn TERAUTENTIKASI + nol konsol, scope logon-scoped jujur; **M5.5 re-scoped** dari WinSW-Service ke Task-Scheduler-autostart, gate artefak I-34 didesain DULU sebelum render; header ringkasan M5).
  **Next konkret (butuh owner + Windows asli, LIVE-only):** **implementasi M5.5 re-scoped** — (1) desain gate artefak DULU (I-34): `.ps1` → `test/ps1-encoding.test.ts` (G-44); Task Scheduler XML → gate well-formed + assert `LogonTrigger` + `<UserId>` current-user + restart-on-failure + `<Hidden>true`; (2) render `deploy/windows/acca-daemon.task.xml` + `scripts/install-windows.ps1`/`uninstall-windows.ps1`; (3) **[LIVE]** logout→login auto-start + SIGKILL→restart <30s + nol konsol + **bukti I-33-proof** (`acca status` DB sama + `claude` resume terautentikasi). Sandbox Cowork **tak bisa** registrasi/verifikasi Task Scheduler → wajib output mesin asli. **Git = owner di terminal** (stage eksplisit 3 file docs, jangan `-A`).
- **Terakhir diupdate:** 2026-07-17 (sesi **Ubuntu**, dgn user — `/session-start` → **M5.4 systemd DITUTUP: SANDBOX + LIVE parsial**) — **585 test hijau** (+15; 2 skip POSIX). Sesi ini di mesin yang benar (Linux) untuk slice yang owner sengaja tunda ke Ubuntu.
  **Baseline:** Step 0 sinkron origin 0/0, tree bersih, nol daemon acca jalan.
  **(1) Sub-task 1 — gate DIDESAIN DULU (pelajaran I-34), lalu render [SANDBOX]:** `test/systemd-unit.test.ts` (struktur semua `*.service`/`*.timer`; template daemon di-**render** [substitusi `<NODE>`/`<ENTRYPOINT>` nilai contoh] → assert `<…>` nol + `Restart=on-failure` + `RestartSec` numerik + `Type=simple` + `WantedBy=default.target`; **+ tiap placeholder wajib disubstitusi `install-linux.sh`** = celah I-34 SEBENARNYA [template↔substitusi tak pernah di-gate, bukan sekadar "bisa di-parse"]) + `test/shell-script.test.ts` (floor lintas-OS: LF-only/shebang/no-BOM + `sh -n` depth, skip HANYA Windows-tanpa-sh). Artefak: `deploy/linux/acca-daemon.service` + `scripts/install-linux.sh` (sed-substitusi + `enable --now` + `enable-linger`). **Keputusan jujur:** assertion "no em-dash" (kelas G-44) **TIDAK** disalin — systemd/POSIX-sh UTF-8 native, em-dash di komentar tak merusak parser (beda .ps1/CP1252); menyalin gate lintas-format tanpa mekanisme = false-premise. **585 test (+15); 3 negative control terbukti KONKRET** (CRLF→fail, `sh -n` sintaks→fail, baris-tanpa-`=`→fail, placeholder `<ORPHAN>` bocor→fail, `Restart=always`→fail; revert→hijau).
  **(2) Sub-task 2 — LIVE di mesin ini → AC-M5-1 PENUH:** install→`active (running)`; **auto-restart** SIGKILL→restart PID baru ~5s (`NRestarts=1`, <30s) — **catatan G-47:** `Restart=on-failure` TAK restart pada SIGTERM/exit-bersih, uji WAJIB SIGKILL; **same-DB (kontras I-33)** `acca status`→"daemon HIDUP" = CLI & service systemd berbagi `acca.db` sama (di Linux user-sama → nol split-brain LocalSystem); `Linger=yes`. **Owner uji langsung: logout→login + reboot → daemon auto-start** (korroborasi Opus: `up 1min`, daemon `active since` +6s pasca-boot, PID baru 2642, `enabled`+linger bertahan). **AC-M5-1 ✅ PENUH.** Daemon **dibiarkan terpasang** (keputusan owner) — build ter-proteksi I-35. **AC-M5-3 ✅ (17 Jul):** job `probe` pending di-stage pada sesi EXITED (guard I-35 → no-op aman, nol resume/inject), daemon di-stop, owner reboot → daemon auto-start → recovery-on-start (AC-7) fire job **7s pasca-boot** (`skipped:probe_stale_status`, created_at>boot) + job terkonsumsi. **Semua AC Linux M5.4 HIJAU** (sesi uji `m53t`+event dibiarkan = jejak append-only). Paruh Windows AC-M5-2 = M5.5 (ditunda/I-33).
  **(3) Wart diperbaiki:** komentar template memuat token `<NODE>`/`<ENTRYPOINT>` literal → sed menggarablnya saat render (**G-48**); reword komentar → re-render bersih (install idempoten).
  **Verifikasi (Opus sendiri):** `npm run check` typecheck+lint+**585 test**. Tier-1 self-review PASS (least-privilege `--user` bukan root = kontras I-33; G-12 path absolut; `Type=simple` cocok daemon foreground; blind-spot penulis=reviewer di-flag → negative control konkret). **Docs:** MILESTONES (AC-M5-1 PARSIAL + header slice ✅), ISSUES (I-34 `.service`/`.sh` ✅ ditutup, XML/M5.5 tetap terbuka tapi ditunda), GOTCHAS (**G-47**+**G-48**), CLAUDE.md/README (570→585 + M5.4), CONTEXT (ini).
  **✅ M5.6 wrap-up DIJALANKAN sesi ini (skill `milestone-wrapup`): M5 DITUTUP PARSIAL.** Regresi 585 hijau + build OK; security T-L1/2/4/5/8 tutup + **T-L7 Linux tutup** (M5.4 LIVE user-scope) + T-L6 residual (restore LIVE belum); CHANGELOG.md dibuat; MILESTONES status M5 + THREAT-MODEL §8.4 diperbarui. Checklist web-app (Zod/RLS/visual/a11y/PII) = **N/A** (CLI lokal, nol UI/endpoint/data-pribadi).
  **✅ T-L6 restore LIVE DITUTUP sesi ini (M5.6):** backup-saat-live (`integrity_check: ok` → I-32 fail-safe tahan) → marker `tl6x` ditambah ke DB live → restore prosedur terdokumentasi → daemon start bersih + `m53t` terjaga + **`tl6x` ter-revert** (bukti restore genuine) + integrity ok. **Gate security-review ke M-remote kini BERSIH** (semua T-L lokal Linux tutup). Restore meninggalkan `acca.db` = snapshot A (nol data hilang; `tl6x` sintetis memang dibuang; `m53t` = jejak audit tetap).
  **✅ AC-M5-8 (quick-start lengkap) + AC-M5-9 (gate) ditutup untuk Linux:** dibuat `scripts/uninstall-linux.sh` (ter-gate shell) + **round-trip LIVE** (uninstall→unit hilang+inactive→reinstall→active+enabled+HIDUP; berakhir TERPASANG). README quick-start dirujuk ke script. **Semua AC M5 HIJAU kecuali AC-M5-2** (Windows service = M5.5/I-33, ditunda). AC-M5-4/5 (security M5.3) juga ditandai (checkbox tertinggal, kini jujur).
  **Next konkret (M5 tutup parsial, gate M-remote bersih):** (1) **M-remote tier A** (Notifier→Telegram — milestone berikutnya per roadmap; butuh bot-token infra-secret + gate dependency grammy 1.44.0 + egress live `api.telegram.org` → butuh owner hadir); (2) **I-35 sisa** (P1 terbuka: probe verifikasi eksplisit — keputusan desain owner overload `probe` vs job `verify`) ATAU buktikan repaint CC limit-diam; (3) **M5.5/I-33** (Windows Service, opsional, butuh owner+Win); (4) **I-32** online-backup API (robustness non-blocking). **Daemon acca tetap terpasang & jalan** (systemd --user, build I-35-protected).
