...
- [~] **AC-M5-2** *(DI-REVISI ADR-026 — jalur autostart per-user, bukan Windows Service; restart target DIAMANDEMEN <30s→<90s, 18 Jul)* Autostart Windows per-user (Task Scheduler @logon, run-hidden) **auto-start saat login** + **auto-restart on-crash <90s** (SIGKILL, bukan exit-bersih). **Amandemen restart (18 Jul, keputusan owner):** floor `RestartOnFailure` Task Scheduler = **PT1M (60s)**, tak bisa <30s spt systemd (`RestartSec=5`) → target Windows direlakan ke **<90s** (konsisten ADR-026 yang sudah menerima always-on terdegradasi utk laptop). **Mekanisme restart = watchdog repetisi** (`LogonTrigger` ber-`Repetition PT1M` + `MultipleInstancesPolicy=IgnoreNew`), BUKAN `RestartOnFailure` sendiri — **LIVE 18 Jul membuktikan `RestartOnFailure` TAK me-restart proses yang di-kill** (G-49). **Bukti WAJIB (diperkuat — "task RUNNING" TAK cukup, kelas kegagalan I-33):** (a) daemon baca `acca.db` **yang SAMA** dengan CLI user (`acca status` konsisten, bukan DB kosong baru) **DAN** (b) `claude`/`agy` ter-spawn **TERAUTENTIKASI** (resume nyata jalan, bukan gagal-auth senyap) **DAN** (c) nol jendela konsol (run-hidden terverifikasi). **Scope jujur:** logon-scoped — **tak** dituntut survive-logout / at-boot-pra-login (direlakan untuk profil laptop, ADR-026/ADR-007; truly always-on = Linux M5.4). **✅ LIVE-PARSIAL 18 Jul:** identitas daemon=user (`lab2026zf\ziffa`, bukan SYSTEM) + **same-`acca.db`** (`acca status`→HIDUP di PID hasil-spawn task) + credentials `.credentials.json` terlihat + **watchdog restart ~61s<90s** + nol-jendela (MainWindowHandle=0, conhost tak-visible) — semua terbukti di mesin asli. **Sisa (1 aksi owner):** logout→login mengonfirmasi trigger @logon fire + mata-telanjang nol-flash. *(live-verify Windows asli — sandbox tak bisa registrasi Task Scheduler)*
...
#### M5.5 — Autostart Windows per-user (Task Scheduler @logon) template + skrip + LIVE **[RE-SCOPED ADR-026 — ✅ SANDBOX + LIVE-PARSIAL 18 Jul; sisa: owner logout/login]**
...
> `scripts/install-windows.ps1`/`uninstall-windows.ps1` (ter-gate `ps1-encoding` G-44). **LIVE-parsial: lihat AC-M5-2**
> (identitas user + same-DB + creds + watchdog-restart ~61s + nol-flash **terbukti**; @logon-fire = sisa owner). **2 temuan
> LIVE mengubah artefak:** (1) `RestartOnFailure` TAK andal restart proses di-kill → **watchdog repetisi + IgnoreNew** (G-49);
> (2) XML comment `--` ditolak parser sungguhan (G-50). **615 test hijau.**
