...
Fase: **M5 ✅ TUTUP PENUH (Linux + Windows) — Hardening + Deploy sebagai service.** (SPEC LOCKED 17 Jul; M5.1 backup + M5.2 skrip/restore + M5.3 security-pass ✅; M5.4 ✅ LIVE penuh [systemd --user]; M5.5 ✅ LIVE penuh 18 Jul [autostart per-user Task Scheduler @logon, ADR-026 — AC-M5-2 hijau: daemon=user + same-DB + creds + @logon-fire + watchdog ~65s + nol-jendela via `conhost --headless`, G-49/50/52]; M5.6 wrap-up ✅). **621 test hijau** (2 skip POSIX). **Backlog 18 Jul: I-32 (backup → online backup API) + I-8/I-17 (proximity wiring + dedup) DITUTUP.** **Next: I-35 sisa (P1, job `verify`) / M-remote tier A (Telegram) — keduanya butuh owner.**
