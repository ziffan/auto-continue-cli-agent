...
**Suite hijau — angka-of-record = 689 pass + 4 skip POSIX** (diukur Windows 18 Jul, pasca M-web.1 + `acca prune`). Jumlah test **bergantung-mesin** (gate per-file men-generate test atas working tree); **ini satu-satunya lokasi ber-integer** — doc lain menunjuk ke sini (D-5/RD-5).
