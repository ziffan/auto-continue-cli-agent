...
**Suite hijau — angka-of-record kini dari CI (bukan pengukuran manual): `ubuntu-latest` = 703 pass · `windows-latest` = 700 pass + 2 skip** (run `29726998571`, 20 Jul; skip = `ipc-stale-socket.test.ts`, POSIX-only). Jumlah test **bergantung-mesin** (gate per-file men-generate test atas working tree); **ini satu-satunya lokasi ber-integer** — doc lain menunjuk ke sini (D-5/RD-5). Tiap push ke `main`/PR memverifikasi ulang lintas-OS (`.github/workflows/ci.yml`).
