...
Fase: **M5 ✅ TUTUP PENUH (Linux + Windows)** — hardening + deploy sebagai service (systemd `--user` [Linux] + autostart per-user Task Scheduler @logon [Windows, ADR-026]). **M-web (Web UI monitor, ADR-028) ✅ DITUTUP FORMAL 19 Jul** (security-review gate T-W1..T-W6 LULUS PENUH — read-only, bind 127.0.0.1, proyeksi ter-firewall nol jalur data baru). **Tak ada milestone aktif** — **M-remote (Telegram) DITUNDA tanpa target** (keputusan owner 18 Jul: "fitur remote Claude Code sudah cukup"). Kerja tersisa = **P3 oportunistik** + opsional UX: W-2 (`daemon --web` co-host) · W-3 (polish page). ADR-027 banner LOCKED + splash/inline-badge diimplementasi 18 Jul. M-remote menunggu owner membuka lagi.

**Suite hijau — angka-of-record = 697 pass + 2 skip POSIX** (diukur Windows 19 Jul, pasca gate M-web + W-3). Jumlah test **bergantung-mesin** (gate per-file men-generate test atas working tree); **ini satu-satunya lokasi ber-integer** — doc lain menunjuk ke sini (D-5/RD-5).
